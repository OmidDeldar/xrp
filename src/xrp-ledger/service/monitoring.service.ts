...
export class MonitoringXrpService implements OnModuleInit {
    ...
    async do_subscribe() {
        ...
        if(sub_response.status === "success") {
            ...
            console.log('Error subscribing:',sub_response);
        }
    }

    async api_request(options: any) {
        if (!options.hasOwnProperty("id")) {
          options.id = "autoid_" + (this.autoid_n++)
        }
      
        let resolveHolder;
        this.awiting[options.id] = new Promise((resolve, reject) => {
        ...
        this.awiting[options.id].resolve = resolveHolder;
        return this.awiting[options.id]
      }

      async  pingpong() {
        console.log("Ping...")
        const response = await this.api_request({command: "ping"})
        console.log("Pong!", response)
      }

      async CountXRPDifference(affected_nodes: any, address: string) {
      ...
      async CountXRPReceived(tx: any, address: string) {
        ...
        if (tx.transaction.TransactionType === "Payment") {
          ...
          this.CountXRPDifference(tx.meta.AffectedNodes, address)
        } else {
